    ...
    return (
        <div className="f_r_c  flex-wrap  gap-8 f_links">
            {FooterLinksConfig.map((item) => (
                <a key={item.name} href={item.link} target={item.target} rel="noreferrer" className={'f_16 c_link_g link'}>{`${intl.get(item.nameI18nKey)}`}</a>
            ))}
        </div>
    );
};
...
const FooterSocialMediaLinks = () => (
    <div className="w-full f_r_c f_sm_links">
        <div className={'f_r_c flex-wrap gap-6 links'}>
...
export default ({ layout }) => {
    ...
    return (
        <div className="w-full f_r_c new_footer_container">
