...
import { useNotificationsContext, useTransactionsContext } from '@usedapp/core/dist/esm/src/providers'
import { TransactionStatus, TransactionOptions, TransactionState } from '@usedapp/core/dist/esm/src/model'
import { BigNumber, Contract, errors, Signer, utils } from 'ethers'
import { buildSafeTransaction, getLatestNonce, GNOSIS_SAFE_ABI, SafeTransaction } from '@usedapp/core/dist/esm/src/helpers/gnosisSafeUtils'
import { useEthers } from '@usedapp/core/dist/esm/src/hooks/useEthers'
import { waitForSafeTransaction } from '@usedapp/core/dist/esm/src/helpers/gnosisSafeUtils'
...
export interface DRXTransactionStatus extends TransactionStatus{
    txIdentifier?: any
}
...
export function usePromiseTransaction(chainId: number | undefined, options?: TransactionOptions) {
    const [state, setState] = useState<DRXTransactionStatus>({ status: 'None' })
    const { addTransaction, updateTransaction } = useTransactionsContext()
    const { addNotification } = useNotificationsContext()
    const { library, account } = useEthers()
    let gnosisSafeContract: Contract | undefined = null
    ...
    const promiseTransaction = useCallback(
        async (txIdentifier: any, transactionPromise: Promise<TransactionResponse>, { safeTransaction }: PromiseTransactionOpts = {}) => {
            if (!chainId) return
            let transaction: TransactionResponse | undefined = null
            try {
                setState({ status: 'PendingSignature', chainId, txIdentifier })

                const result = (await isNonContractWallet(library, account))
                    ? await handleNonContractWallet(txIdentifier, transactionPromise)
                    : await handleContractWallet(txIdentifier, transactionPromise, { safeTransaction })
                transaction = result?.transaction
                return result?.receipt
            } catch (e: any) {
                console.debug(`Error:`, e);

                const parsedErrorCode = parseInt(e.error?.data?.code ?? e.error?.code ?? e.data?.code ?? e.code, 10)
                ...
                if (transaction) {
                    ...
                    if (droppedAndReplaced) {
                        ...
                        setState({ txIdentifier, status: 'Fail', transaction, receipt: e.receipt, errorMessage, errorCode, errorHash, chainId })
                    }
                } else {
                    setState({ txIdentifier, status: 'Exception', errorMessage, errorCode, errorHash, chainId })
                }
                return undefined
            }
        },
    ...
    const handleNonContractWallet = async (txIdentifier: any, transactionPromise: Promise<TransactionResponse>) => {
        if (!chainId) return

        const transaction = await transactionPromise

        setState({ txIdentifier, transaction, status: 'Mining', chainId })
        addTransaction({
            transaction: {
                ...transaction,
                chainId: chainId,
        ...
        setState({ txIdentifier, receipt, transaction, status: 'Success', chainId })
    ...
    const handleContractWallet = async (
        ...
        setState({ txIdentifier, status: 'CollectingSignaturePool', chainId })
